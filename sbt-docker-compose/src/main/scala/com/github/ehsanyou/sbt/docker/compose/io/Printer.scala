...
import com.github.ehsanyou.sbt.docker.compose.io.Printer._
import io.circe._
...
/**
  * This class is just a copy of `circe-yaml` `Printer` class, with a small fix regarding representing numbers.
  * `circe-yaml`'s `Priner` class generates a valid `yaml` but for some reason `docker-compose` can't parse numeral values generated by it.
  */
final case class Printer(
  ...
  private def jsonToYaml(json: Json): Node = {
    ...
    json.fold(
      ...
      str => stringNode(str),
      arr => new SequenceNode(Tag.SEQ, arr.map(jsonToYaml).asJava, sequenceStyle == FlowStyle.Flow),
